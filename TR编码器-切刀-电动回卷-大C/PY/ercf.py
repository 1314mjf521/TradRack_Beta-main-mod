...
class Ercf:
    ...
    ENCODER_MIN = 2.5           # The threshold (mm) that determines real encoder movement (ignore erroneous pulse)
...
####################################
    ...
    def _seconds_to_human_string(self, seconds):
        ...
        if hours >= 1 or minutes >= 1:
            result += "%d 分钟 " % minutes
        result += "%d 秒" % int((math.floor(seconds) % 60))
        return result

    def _swap_statistics_to_human_string(self):
        msg = "ERCF Statistics:"
        msg += "\n%d 次交换 已完成" % self.total_swaps
        msg += "\n%s 时间 加载 (平均: %s)" % (self._seconds_to_human_string(self.time_spent_loading),
                                                    self._seconds_to_human_string(self.time_spent_loading / self.total_swaps) if self.total_swaps > 0 else "0")
        msg += "\n%s 时间 卸载 (平均: %s)" % (self._seconds_to_human_string(self.time_spent_unloading),
                                                      self._seconds_to_human_string(self.time_spent_unloading / self.total_swaps) if self.total_swaps > 0 else "0")
        msg += "\n%s 时间 暂停 (总暂停次数: %d)" % (self._seconds_to_human_string(self.time_spent_paused), self.total_pauses)
        return msg
    ...
    def _state_to_human_string(self, direction=None):
        ...
        sensor_str = " [传感器] " if self._has_toolhead_sensor() else ""
        ...
        if self.tool_selected == self.TOOL_BYPASS and self.loaded_status == self.LOADED_STATUS_FULL:
            ...
            visual = "ERCF [T%s] >>>>> [编码器] >>>>>>>>>>>>> [挤出机] >>>%s>>> [喷嘴] 加载" % (tool_str, sensor_str)
            visual += counter_str
...
### LOGGING AND STATISTICS FUNCTIONS GCODE FUNCTIONS
    ...
    cmd_ERCF_RESET_STATS_help = "重置ERCF统计信息"
    ...
    cmd_ERCF_DUMP_STATS_help = "转储ERCF统计信息"
    def cmd_ERCF_DUMP_STATS(self, gcmd):
        if self._check_is_disabled(): return
        self._dump_statistics(report=True)

    cmd_ERCF_SET_LOG_LEVEL_help = "设置ERCF的日志级别"
...
### SERVO AND MOTOR GCODE FUNCTIONS
    ...
    cmd_ERCF_MOTORS_OFF_help = "关闭两个ERCF电机"
    def cmd_ERCF_MOTORS_OFF(self, gcmd):
        if self._check_is_disabled(): return
        self._servo_up()
        self._motors_off()

    cmd_ERCF_BUZZ_GEAR_MOTOR_help = "ERCF齿轮电机发出嗡嗡声"
    ...
    cmd_ERCF_SYNC_GEAR_MOTOR_help = "将ERCF齿轮电机与挤出机电机同步"
...
### CALIBRATION GCODE COMMANDS
    ...
    cmd_ERCF_CALIBRATE_help = "完成所有ERCF工具的校准"
    ...
    cmd_ERCF_CALIBRATE_SINGLE_help = "单个ERCF工具的校准"
    ...
    cmd_ERCF_CALIBRATE_ENCODER_help = "ERCF编码器的校准程序"
    ...
    cmd_ERCF_CALIBRATE_SELECTOR_help = "校准已定义闸门的选择器位置"
...
### STATE GCODE COMMANDS
    ...
    cmd_ERCF_ENABLE_help = "启用ERCF功能并重置状态"
    ...
    cmd_ERCF_DISABLE_help = "禁用所有ERCF功能"
    def cmd_ERCF_DISABLE(self, gcmd):
        if self.is_enabled:
            self._log_always("ERCF disabled")
            self.is_enabled = False

    cmd_ERCF_ENCODER_help = "ERCF编码器手动启用/禁用控制"
    ...
    cmd_ERCF_RESET_help = "忘记持久状态并重新初始化默认值"
...
#############################
    ...
    def _unload_bowden(self, length, skip_sync_move=False):
        ...
        self.gcode.run_script_from_command("REWIND_ON_SLOW")
        self.filament_direction = self.DIRECTION_UNLOAD
        ...
        self.gcode.run_script_from_command("REWIND_ON")
        moves = 1 if length < (self._get_calibration_ref() / self.num_moves) else self.num_moves
        delta = 0
        for i in range(moves):
    ...
    def _unload_encoder(self, max_length):
        ...
        for i in range(max_steps):
            ...
            if delta >= self.encoder_move_step_size * 0.2: # 20 %
                for i in range(max_steps):
                    if delta >= self.encoder_move_step_size * 0.3: # 30 %
                        self._set_loaded_status(self.LOADED_STATUS_PARTIAL_BEFORE_ENCODER)
                        park = 9 - delta # will be between 8 and 20mm (for 23mm parking_distance, 15mm step)
                        self.gcode.run_script_from_command("REWIND_OFF")
                        delta = self._trace_filament_move("Final parking", -park)
                        self.gcode.run_script_from_command("_ERCF_CUT")
                        for i in range(max_steps):
                            msg = "Unloading step #%d from encoder" % (i+1)
                            self.gcode.run_script_from_command("REWIND_ON_SLOW")
                            delta = self._trace_filament_move(msg, -self.encoder_move_step_size)
...
### CORE GCODE COMMANDS ##########################################################
    ...
    cmd_ERCF_SELECT_help = "选择指定的工具或通道"
    ...
    cmd_ERCF_CHANGE_TOOL_help = "执行工具交换"
    ...
    cmd_ERCF_LOAD_help = "在当前工具/通道上加载耗材，或者可选地仅加载挤出机以用于旁路或回收用途"
    ...
    cmd_ERCF_EJECT_help = "弹出耗材并将其停放在ERCF中，或者可选地仅卸载挤出机"
    ...
    cmd_ERCF_SELECT_BYPASS_help = "选择耗材旁路"
    ...
    cmd_ERCF_LOAD_BYPASS_help = "已弃用。改为使用旁路感知ERCF_LOAD"
    ...
    cmd_ERCF_UNLOAD_BYPASS_help = "已弃用。改为使用旁路感知ERCF_EJECT"
    ...
    cmd_ERCF_PAUSE_help = "暂停当前打印并锁定ERCF操作"
    ...
    cmd_ERCF_RESUME_help = "环绕默认RESUME宏"
    ...
    # Not a user facing command - used in automatic wrapper
    cmd_ERCF_CANCEL_PRINT_help = "环绕默认CANCEL_PRINT宏"
    ...
    cmd_ERCF_RECOVER_help = "手动干预/移动后恢复耗材位置并设置ERCF状态"
...
### GCODE COMMANDS INTENDED FOR TESTING #####################################
    ...
    cmd_ERCF_SOAKTEST_SELECTOR_help = "选档器运动试验"
    ...
    cmd_ERCF_SOAKTEST_LOAD_SEQUENCE_help = "测试工具加载/卸载顺序"
    ...
    cmd_ERCF_TEST_GRIP_help = "测试工具的ERCF夹力"
    ...
    cmd_ERCF_TEST_SERVO_help = "测试伺服角度"
    ...
    cmd_ERCF_TEST_MOVE_GEAR_help = "移动ERCF齿轮"
    ...
    cmd_ERCF_TEST_LOAD_help = "测试从ERCF到挤出机的耗材负载"
    ...
    cmd_ERCF_TEST_TRACKING_help = "测试齿轮进给和编码器感应的跟踪"
    ...
    cmd_ERCF_TEST_UNLOAD_help = "用于测试耗材卸载的精细控制，并将其停放在ERCF中"
    ...
    cmd_ERCF_TEST_HOME_TO_EXTRUDER_help = "测试从波顿末端将耗材复位到挤出机。用于校准电流减少或安装保护阈值"
    ...
    cmd_ERCF_TEST_CONFIG_help = "出于测试或印刷调整目的对ERCF配置进行运行时调整"
...
### GCODE COMMANDS FOR RUNOUT, TTG MAP, GATE MAP and GATE LOGIC ##################################
    ...
    cmd_ERCF_ENCODER_RUNOUT_help = "内部编码器灯丝跳动处理程序"
    ...
    cmd_ERCF_ENCODER_INSERT_help = "内部编码器灯丝检测处理器"
...
#            self._pause(str(ee))
    ...
    cmd_ERCF_DISPLAY_TTG_MAP_help = "显示工具到ERCF通道位置的当前映射。与无尽耗材一起使用"
    ...
    cmd_ERCF_REMAP_TTG_help = "将工具重新映射到特定通道并设置通道可用性"
    ...
    cmd_ERCF_SET_GATE_MAP_help = "定义每个通道上耗材的类型和颜色"
    ...
    cmd_ERCF_ENDLESS_SPOOL_help = "重新定义EndlessSspool组"
    ...
    cmd_ERCF_CHECK_GATES_help = "自动检查通道、停车耗材并标记可用性"
    ...
    cmd_ERCF_PRELOAD_help = "在指定或现在的通道处预加载灯丝"
