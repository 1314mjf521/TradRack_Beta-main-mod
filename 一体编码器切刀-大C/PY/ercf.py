...
class Ercf:
    ...
    def __init__(self, config):
        ...
        self.parking_distance = config.getfloat('parking_distance', 23., minval=12., maxval=50.)
...
### SERVO AND MOTOR GCODE FUNCTIONS
    ...
    cmd_ERCF_SERVO_UP_help = "脱开ERCF齿轮"
    def cmd_ERCF_SERVO_UP(self, gcmd):
        if self._check_is_disabled(): return
        if self._check_is_paused(): return
        self._servo_up()

    cmd_ERCF_SERVO_DOWN_help = "接合ERCF齿轮"
...
### CORE GCODE COMMANDS ##########################################################
    ...
    cmd_ERCF_SELECT_help = "选择指定的刀具或浇口"
    ...
    cmd_ERCF_LOAD_help = "将灯丝加载到当前通道上，或者可选地仅加载挤出机以供旁路或回收使用"
    ...
    cmd_ERCF_EJECT_help = "弹出细丝并将其停放在ERCF中，或者可选地仅卸载挤出机"
    ...
    cmd_ERCF_SELECT_BYPASS_help = "选择灯丝旁路"
    ...
    cmd_ERCF_RECOVER_help = "手动干预/移动后，恢复灯丝位置并设置ERCF状态"
...
### GCODE COMMANDS INTENDED FOR TESTING #####################################
    ...
    cmd_ERCF_TEST_GRIP_help = "测试工具的ERCF夹具"
    ...
    cmd_ERCF_TEST_LOAD_SEQUENCE_help = "测试顺序"
    ...
    cmd_ERCF_TEST_LOAD_help = "测试从ERCF到挤出机的长丝负载"
    ...
    cmd_ERCF_TEST_UNLOAD_help = "用于测试细丝卸载和将其停放在ERCF中的精细控制"
    ...
    cmd_ERCF_TEST_HOME_TO_EXTRUDER_help = "测试从波顿末端将灯丝复位到挤出机.用于校准电流减少或失速防护阈值"
...
#            self._pause(str(ee))
    ...
    cmd_ERCF_DISPLAY_TTG_MAP_help = "显示刀具到ERCF浇口位置的当前映射。与环形线轴一起使用"
    ...
    cmd_ERCF_REMAP_TTG_help = "将工具重新映射到特定浇口并设置浇口可用性"
    ...
    cmd_ERCF_SET_GATE_MAP_help = "定义每个门上灯丝的类型和颜色"
    ...
    cmd_ERCF_CHECK_GATES_help = "自动检查闸门、停车灯丝和标记可用性"
    ...
    cmd_ERCF_PRELOAD_help = "在规定或电流门处预加载灯丝"
